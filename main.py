...
def generate_clinical_data():
    ...
    tumor_data = {
        ...
        '肿瘤类型': np.random.choice(['非小细胞肺癌', '黑色素瘤', '肾细胞癌', '尿路上皮癌'], n_patients),
...
class AdvancedPredictiveModel:
    ...
    def prepare_features(self, df, fit_scaler=False):
        ...
        self.feature_columns = [
            ...
            '既往治疗线数', '转移部位数', '肝转移', 'PD-L1编码', 'TMB(mut/Mb)',
            'NLR', '白蛋白(g/L)', 'LDH(U/L)', 'CRP(mg/L)'
        ]
...
with st.sidebar:
    ...
    page = st.radio(
        "",
        ["🏥 临床数据总览", "🎯 智能预测系统", "📊 模型性能分析", "📈 生存分析", "🔬 生物标志物分析"]
    )
...
if page == "🏥 临床数据总览":
    ...
    tab1, tab2, tab3 = st.tabs(["数据预览", "患者分布", "相关性分析"])
...
# ==================== 页面2：智能预测系统 ====================
    ...
    with st.expander("📝 输入患者信息", expanded=True):
        ...
        with col1:
            ...
            metastasis = st.number_input("转移部位数", 0, 5, 1)
    ...
    if st.button("🔮 开始预测", type="primary", use_container_width=True):
        ...
        input_data['PD-L1编码'] = input_data['PD-L1表达'].map({'阴性':0, '低表达':1, '高表达':2})
        input_data['肿瘤类型编码'] = 0
...
# ==================== 页面4：生存分析 ====================
    ...
    st.subheader("中位生存时间")
    col1, col2, col3, col4 = st.columns(4)
    doses = sorted(df['剂量水平(mg/kg)'].unique())
    cols = [col1, col2, col3, col4]
    for i, dose in enumerate(doses):
        with cols[i]:
            median = df[df['剂量水平(mg/kg)'] == dose]['PFS_月'].median()
